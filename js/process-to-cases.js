...
(function () {
  ...
  /* THE CLIPPED COPY.
     ...
     This used to be .process__list — the five-stage column standing BESIDE
     the camera, whose visible band had to be found by measuring the last
     stage item in it. Section 05's copy now lives ON the cards inside the
     camera (see index.html), so the only sibling copy left is the section
     header, and it is a single compact block: what is on screen IS its own
     rect, with no sub-band to isolate. */
  var copy = process.querySelector('.process__header');
  var source = camera;
  ...
  if (!camera || !frame || !copy || !featured || !reveal || !head || !rise) return;
  ...
  /* THE SETTLE — the beat before anything moves.

     The pin starts at 'bottom bottom', i.e. when the END of #process reaches
     the bottom of the fold. That was written when the section was TALLER than
     the viewport, so the reader had already scrolled through the whole stage
     list before the pin could engage, and the composition had genuinely been
     standing still for a while by then (the "no opening pause" note further
     down rests on exactly that assumption).

     It no longer holds. #process is now one viewport tall, so 'bottom bottom'
     resolves to the same scroll position as 'top top': the pin engages the
     instant the section arrives, .is-morphing goes on ~44px later, and the
     section header starts fading before the reader has had a chance to read
     the words "One case, five stages" at all. The transition ran on arrival
     rather than on departure.

     SETTLE_VH puts the missing beat back as real scroll distance. It is added
     to the pin's budget and consumed by an empty leading tween (see the settle
     hold in build()), so for this stretch the section is pinned, fully opaque
     and completely static — the reader reads the heading and watches the
     marquee run, exactly as if it were an ordinary section — and only then
     does the close begin.

     Pinned rather than simply starting the pin later: the marquee has to keep
     running and the section has to stay put while the reader takes it in, and
     a later start would let the section scroll up and out of the fold before
     the morph could take hold of it. */
  var SETTLE_VH = 0.6;
  ...
  /* The settle is part of the pin's budget: it is scroll the reader spends
     ...
     distance or the pin would run past the spacer it is standing on. */
  function pinDistance() {
    return Math.round(window.innerHeight * (SETTLE_VH + TRAVEL_VH + TAIL_VH));
  }
  ...
  /* THE SETTLE IS NOT PART OF THE REVEAL'S GEOMETRY.

     `distance` is the pin's whole budget, and the settle is now inside it —
     but the settle is scroll during which NOTHING moves, so Case studies must
     not be pulled up through it. Left uncorrected the pull grew by the
     settle's full height and dragged Case studies up with it: by the time the
     object landed, the section had already scrolled a fifth of a viewport past
     the top of the fold, which is the opposite of the arrival the REVEAL_VH
     note above is protecting.

     Subtracting the settle here measures the pull from the moment the close
     actually begins rather than from the pin's first pixel, so Case studies
  ...
  function openCamera() {
    ...
    /* THERE IS NO STAGE HOLD TO DROP ANY MORE.

       js/process-stages.js used to pin the small right-hand picture beside
       the five-stage column by writing a translateY to the camera, and that
       translate (~2600px by the end of the list) had to be released through
       its own hook here or it would compose with the morph's transform and
       drag the closing object straight off screen.

       Section 05 is now a continuously running band (js/process-marquee.js)
       with no scroll-driven hold of any kind: the camera carries no transform
       of its own before the pin engages, so there is nothing to cancel and
       GSAP starts from a clean element. The marquee writes its transform to
       .process__track, which is INSIDE the camera and therefore inside the
       clip — it rides the close rather than fighting it. */
    ...
    /* PULL THE STAGE TO THE SCREEN'S LEFT EDGE.
       ...
       against .process__body, not the page.
  ...
  function build() {
    ...
    section.style.marginBottom = (-revealPull(dist, SETTLE_VH)) + 'px';
    ...
    /* --- PHASE ONE, 0.16 -> CLOSE_END  the frame closes IN PLACE ----------
       ...
       where it closes to. */
    /* THE SETTLE'S SPAN, in the timeline's own units.

       The whole timeline is written in units where 1 is the travel span, so
       the settle — which is measured in viewports, like TRAVEL_VH — has to be
       converted into the same currency before it can be used as an offset.
       SETTLE_VH / TRAVEL_VH is that conversion, exactly as TAIL_SPAN converts
       TAIL_VH a few lines above.

       Everything downstream is already expressed relative to CLOSE_IN, so
       giving CLOSE_IN a non-zero value is all it takes to push the entire
       choreography back by the settle: the close, the wash, the caption, the
       travel and the copy reveal all shift with it, and their durations and
       relative spacing are untouched. */
    var SETTLE_SPAN = SETTLE_VH / TRAVEL_VH;

    /* The close no longer starts on the pin's first pixel — it starts after
       the settle. See SETTLE_VH at the top of this file: the section is one
       viewport tall now, so the pin engages the moment it arrives and the old
       'the frame arrives already still' assumption (the note where the opening
       pause used to be) is no longer true. The settle is what makes it true
       again, by holding the section still and fully opaque for its duration
       before anything begins to move. */
    var CLOSE_IN = SETTLE_SPAN;
    ...
    var CLOSE_END = SETTLE_SPAN + 0.92;
    ...
    /* (1 - CLOSE_END) was the run from the close's end to the timeline's
       nominal 1. With both shifted by the settle that distance is now
       (SETTLE_SPAN + 1) - CLOSE_END. Named once here because three separate
       tweens below derive their durations and offsets from it, and an
       un-shifted copy left in any of them would silently shorten phase two by
       the settle's whole length. */
    var PHASE_TWO = (SETTLE_SPAN + 1) - CLOSE_END;
    var TRAVEL_END = CLOSE_END + PHASE_TWO + TAIL_SPAN * 0.8;
    ...
    var END = SETTLE_SPAN + 1 + TAIL_SPAN;
    ...
    tl.to(source, {
      ...
      duration: (PHASE_TWO + TAIL_SPAN * 0.8) * 0.72,
      ease: 'none'
    }, CLOSE_END);
    ...
    tl.to(source, {
      ...
      duration: (PHASE_TWO + TAIL_SPAN * 0.8) * 0.28,
      ...
      ease: 'none'
    }, CLOSE_END + (PHASE_TWO + TAIL_SPAN * 0.8) * 0.72);
    ...
    tl.to({}, { duration: TAIL_SPAN }, SETTLE_SPAN + 1);
    ...
    tl.eventCallback('onUpdate', function () {
      /* MIRROR --wash ONTO THE SECTION.
         ...
         it (.process__header, see style.css) sits in a
      ...
      /* THE COPY IS CLIPPED BY THE SAME BOUNDARY, NOT FADED.
         ...
         .process__header is a SIBLING of the camera, not a child, so the
         ...
         the header wear the identical rectangle, so the closing edge sweeps
      ...
      if (n && n.length >= 4) {
        ...
        var lr = copy.getBoundingClientRect();

        /* THE HEADER IS ITS OWN VISIBLE BAND.

           With the old five-stage column this had to solve about the last
           item's rect: the column was ~2790px tall, most of it far above the
           fold, so proportional insets taken off its own box bit the copy
           immediately and then barely moved. The header is a two-line block
           that is entirely on screen when the close runs, so its own rect IS
           the visible band and the same proportion can be applied about its
           centre directly. */
        var vr = lr;
