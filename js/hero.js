...
(function () {
  ...
  if (header) {
    ...
    function syncHeader() {
      ...
      /* Logo visibility. The brand is part of the HERO composition, not a
         ...
         wordmark is redundant (the user knows whose site they are on) and it
         competes with the section headings passing under it. Past STUCK_AT
         the mark fades out and only the hamburger remains; scrolling back to
         the top brings it back. The <a class="brand"> stays in the DOM and
         in the tab order — only its ink is dropped — so "back to top" is
  ...
  if (heroEl) {
    ...
    /* The reveal is additionally gated behind the preloader when one is
       running. Without this the entrance would play UNDERNEATH the loader and
       be over by the time the loader lifted, so the hero would arrive already
       at rest instead of animating in as the video expands.

       preloader.js publishes window.__preloaderGate and ALWAYS resolves it —
       including on its own failure paths. When that file is absent the gate is
       undefined and gated() falls through to calling reveal() directly, which
       is the original behaviour unchanged. */
    function gated(fn) {
      var gate = window.__preloaderGate;
      if (gate && typeof gate.then === 'function') {
        gate.then(fn, fn);   // resolve OR reject must both release the hero
      } else {
        fn();
      }
    }
    ...
    if (document.fonts && document.fonts.ready) {
      document.fonts.ready.then(function () { gated(reveal); });
    }
    // Hard floor — never leave it hidden. Still gated: the floor exists for a
    // font that never settles, not to punch through the intro.
    window.setTimeout(function () { gated(reveal); }, 900);
    // Absolute floor, ungated. If the preloader promise somehow never settles
    // the hero must still appear; 7000ms matches the preloader's own failsafe.
  ...
  /* Scoped to ONE display block. The hero now carries two of them (the H1 and
     the CREW HEALTHCARE alt block) and they must split independently: their
     per-character transition delays both have to start from zero, or the alt
     block's wipe would inherit the H1's row offsets and start mid-stagger.
  ...
  function splitRows(scope) {
    ...
    /* Rows inside a cycling phrase that is NOT the current one are skipped.
       Splitting the whole alt block in one pass would split all four phrases
       up front, and the three that are waiting their turn would be left with
       their characters parked off to the right for good — the block they sit
       in is never wiped in again, only the phrase is. Each phrase is instead
       split by showPhrase at the moment it becomes current.

       A row inside the CURRENT phrase still splits here, so the first phrase
       arrives as part of the alt block's own wipe-in. */
    var rows = Array.prototype.filter.call(all, function (n) {
      var item = n.closest ? n.closest('.hero__cycle-item') : null;
      return !item || item.classList.contains('is-current');
    });
    if (!rows.length) return;

    // Already split. The alt block and each cycling phrase are separate
    // scopes, so this guard applies per scope and is what keeps a repeat call
    // from wrapping masks inside masks.
    if (rows[0].querySelector('.hero__cmask')) return;
    ...
    /* ONLY the real H1 gets one, and it is identified positively rather than
       by excluding the other cases. Excluding by class missed the cycling
       phrases: each is its own split scope, so each was handed a copy of its
       own text and the row rendered the phrase twice. Both the alt block and
       every phrase inside it are aria-hidden decoration — a screen-reader
       copy there would announce a second headline that rewrites itself. */
    var h1 = (container.classList && container.classList.contains('hero__display') &&
              !container.classList.contains('hero__display--alt')) ? container : null;
    if (h1 && h1.querySelector && !h1.querySelector('.hero__sr') && srText) {
  ...
  // Wrapped rather than passed directly: document.fonts.ready resolves WITH
  // the FontFaceSet, which would arrive as splitRows' scope argument and send
  // it looking for rows inside an object that has no querySelectorAll.
  function splitH1() { splitRows(); }
  if (document.fonts && document.fonts.ready) {
    document.fonts.ready.then(splitH1);
  } else {
    splitH1();
  }
  window.setTimeout(function () { if (!chars.length) splitH1(); }, 900);

  /* --- 2c. The second display state + its cycling row --------------------- */

  /* Two things happen here, in order:

       1. THE SWAP. Once the H1's character entrance has finished and held,
          the H1 wipes out and the CREW HEALTHCARE block wipes in, running the
          same per-character mask animation so the second headline arrives the
          way the first one did rather than just appearing.

       2. THE CYCLE. From then on the third row rewrites itself every few
          seconds through the four positioning lines, wiping the outgoing
          phrase out and the incoming one in with that same animation.
  ...
  if (altEl && h1El) {
    ...
    var cycleItems = altEl.querySelectorAll('.hero__cycle-item');
    var cycleIdx   = 0;
    var cycleTimer = null;
    ...
    /* Re-runs the per-character wipe on a scope that is already split.
       ...
       .hero-ready lives on <html> and is what releases every character at
       once, so a second block cannot simply be handed that class later — by
       the time the block is split the class is already on, and its characters
       would be released before they were ever parked. Instead they are parked
       back at the start offset inline and released on the next frame, which
       reproduces the same transition locally. */
    function wipeIn(scope) {
      var cs = scope.querySelectorAll('.hero__char');
      if (!cs.length) return;
      if (reduce.matches) {
        Array.prototype.forEach.call(cs, function (c) {
          c.style.transition = 'none';
          c.style.transitionDelay = '0s';
          c.style.setProperty('--c-x', '0%');
        });
        return;
      }
      // Park.
      Array.prototype.forEach.call(cs, function (c) {
        c.style.transition = 'none';
        c.style.setProperty('--c-x', '100%');
      });
      // Force the parked state to be committed before the release, or the
      // browser collapses both writes into one recalc and nothing animates.
      void scope.offsetWidth;
      requestAnimationFrame(function () {
        Array.prototype.forEach.call(cs, function (c) {
          // Cleared, not restated: the transition and the per-character delay
          // both come back from the stylesheet rule, so the cadence stays the
          // one the entrance already uses.
          c.style.transition = '';
          c.style.setProperty('--c-x', '0%');
        });
      });
    ...
    /* Parks every character to the right WITHOUT animating, to prime a block
       about to become visible — otherwise it paints fully-formed for one
       frame before wipeIn parks it. */
    function park(scope) {
      if (reduce.matches) return;
      Array.prototype.forEach.call(scope.querySelectorAll('.hero__char'), function (c) {
        c.style.transition = 'none';
        c.style.setProperty('--c-x', '100%');
      });
    }

    /* Wipes a scope's characters OUT to the left, the mirror of the entrance.
       Returns how long the exit takes so the caller can schedule what follows
       instead of guessing at it. */
    function wipeOut(scope) {
      var cs = scope.querySelectorAll('.hero__char');
      if (!cs.length || reduce.matches) return 0;
      Array.prototype.forEach.call(cs, function (c, i) {
        c.style.transition = 'transform 0.6s cubic-bezier(0.7, 0, 0.84, 0)';
        c.style.transitionDelay = (i * 0.012).toFixed(3) + 's';
        c.style.setProperty('--c-x', '-110%');
      });
      return 600 + cs.length * 12;
    }

    /* --- the cycling row ------------------------------------------------- */

    /* The row is right-aligned, so the box needs an explicit width for the
       growth between phrases to be animatable — width: auto does not
       transition. The width is MEASURED off the phrase rather than assumed,
       so editing the copy in the markup needs no change here. */
    function lockCycleWidth() {
      if (!cycleBox) return;
      var cur = cycleItems[cycleIdx];
      if (!cur) return;
      // Measured with the box unconstrained, or every phrase after the first
      // is measured inside the PREVIOUS phrase's locked width and the row
      // never grows past whatever the first one happened to need.
      var prev = cycleBox.style.width;
      cycleBox.style.width = 'auto';
      var w = cur.getBoundingClientRect().width;
      cycleBox.style.width = prev;
      void cycleBox.offsetWidth;
      cycleBox.style.width = w + 'px';
    }

    /* Advances to `next`.

       cycleIdx is moved SYNCHRONOUSLY, before the deferred commit, and the
       outgoing element is captured in a local. Advancing it inside the
       timeout instead let a second tick read a stale index while the first
       swap was still waiting out its wipe, and the row then skipped a phrase
       and desynced from the running order (Simplified -> Managed ->
       Compliance-ready -> Simplified -> Optimized). Everything the deferred
       half needs is now bound at call time. */
    function showPhrase(next) {
      if (!cycleItems.length) return;
      var outgoing = cycleItems[cycleIdx];
      var incoming = cycleItems[next];
      if (!incoming || incoming === outgoing) return;

      cycleIdx = next;

      var outMs = outgoing ? wipeOut(outgoing) : 0;

      window.setTimeout(function () {
        // A later tick may already have moved on while this one was waiting;
        // if so this swap is stale and must not paint over the newer phrase.
        if (cycleItems[cycleIdx] !== incoming) return;

        if (outgoing) outgoing.classList.remove('is-current');
        incoming.classList.add('is-current');

        // Split this phrase on first use. Each .hero__cycle-item is its own
        // scope, so splitRows' already-split guard applies per phrase.
        splitRows(incoming);
        park(incoming);
        lockCycleWidth();
        wipeIn(incoming);
      }, reduce.matches ? 0 : Math.min(outMs, 420));
    }

    /* CYCLE_HOLD has to clear the wipe-out wait plus the wipe-in itself, or
       the next tick lands while the current phrase is still arriving. */
    function startCycle() {
      if (cycleItems.length < 2) return;
      window.clearInterval(cycleTimer);
      cycleTimer = window.setInterval(function () {
        showPhrase((cycleIdx + 1) % cycleItems.length);
      }, CYCLE_HOLD);
    }
    ...
    /* The cycle is paused while the hero is off screen — a headline rewriting
       ...
       caught mid-wipe reads as a glitch rather than as a loop. */
    ...
    // Recompute the locked width when the type reflows.
  ...
  if (heroEl) {
    ...
    // hero on its painted ground: reduced-motion, an explicit request for no
    // movement, and an autoplaying loop is exactly the kind of motion that
    // request means. The <source> carries data-src rather than src, so in
    // that case the bytes are never requested at all — not
    ...
    // Re-evaluate if the user changes the motion preference. Only ever adds
    // the source; never re-downloads.
    [reduceMotion].forEach(function (mq) {
  ...
  /* Moved to js/smooth-scroll.js so the inner pages can share it. That file
     is loaded ahead of this one and exposes the same window.__medshieldScroll
     API this file and nav.js already used, so nothing here changed except
...
/* ---------------------------------------------------------------------------
   Scroll-inset panel - REMOVED
   ---------------------------------------------------------------------------
   The contact section used to have its panel width scrubbed on scroll, so its
   side gutters opened as the section crossed the viewport. That animation has
   been removed along with the --cta-p variable it wrote; the section is now a
   static full-width band styled entirely in CSS. Nothing replaces it here. */
